    ...
    bookdown::gitbook:
      config:
        toc:
          ...
          before: <li><h4>Reproducible data treatment with R</h4></li>
          after: null
        toolbar:
          position: fixed
        edit : null
        download: [["tutoR.Rmd","Rmd file"]]
        search: yes
        fontsettings:
          theme: white
          family: sans
...
This class is indented to provide the students with the tools to handle themselves with R, Rmarkdown and Shiny, and _not_ to provide an extensive review of everything that is possible with R. To go further:
...
- The [ggplot cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf)
...
In 2016, Nature conducted a survey of 1576 researchers who took a brief online questionnaire on reproducibility in research. According to the survey, more than 70% of researchers have tried and failed to reproduce another scientist's experiments, and more than half have failed to reproduce their own experiments. [^1]

> "Although 52% of those surveyed agree there is a significant 'crisis' of reproducibility, less than 31% think failure to reproduce published results means the result is probably wrong, and most say they still trust the published literature."[^2]

[^1]: From [wikipedia](https://en.wikipedia.org/wiki/Reproducibility#Reproducible_research)
[^2]: [https://www.nature.com/news/1-500-scientists-lift-the-lid-on-reproducibility-1.19970](https://www.nature.com/news/1-500-scientists-lift-the-lid-on-reproducibility-1.19970)

Replicability and reproducibility are some of the keys to scientific integrity. Establishing a workflow in which your data are always treated in the same manner is thus a way to:

- Reduce error risks inherent to human manipulation
- Keep track of all the treatments you perform on your data and document your methodology
...
It is the objective of this class to provide you the tools necessary to work with this philosophy.
...
[ ![*The Rstudio environment*](Img/Rstudio.png) ](Img/Rstudio.png)
...
If you don't want to use Rstudio but rather want to keep with your favorite text editor, like I do[^3]  ([Sublime Text](https://www.sublimetext.com/3), [Atom](https://atom.io/), [Visual Studio Code](https://code.visualstudio.com/)...)
...
[^3]: I personally use [Sublime Text 3](https://www.sublimetext.com/3) with the following packages:<br>
<ul>
...
[Download](Data/population.csv) the `population.csv` file and save it in your working directory.
...
library(openxlsx) # load openxlsx to read Excel files
read.xlsx("Data/test.xlsx", sheet=1)
read.xlsx("Data/test.xlsx", sheet=2)
```
...
Download [population.txt](Data/population.txt) and load it into a `data.frame`.
...
Write a function returning a list (created if empty, incremented if existing) containing the name, age and gender of the people in the class. Use it to print the list as a list or as a data.frame.

<details>
<summary>Solution</summary>

```{r, warnings=FALSE}
people <- function(ppl=list(), name, age, gender){
    # if the list is not provided, initialize it
    if(length(ppl)==0) ppl <- list() 
    # increment vectors
    ppl[['name']]   <- c(ppl[['name']], name)
    ppl[['age']]    <- c(ppl[['age']], age)
    ppl[['gender']] <- c(ppl[['gender']], gender)
    # return the list
    ppl
}
...
- Download [population.csv](Data/population.csv) and load it into a `data.frame`
...
Further (more detailed) readings [here](http://tutorials.iq.harvard.edu/R/Rgraphics/Rgraphics.html) and on the [cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) for example.
...
- Download the two sample Raman spectra: [PPC60_G_01.txt](Data/PPC60_G_01.txt) and [PPC60_G_30.txt](Data/PPC60_G_30.txt)
...
- Load [exo_fit.txt](Data/exo_fit.txt) in a `data.frame`.
...
- Load the Raman spectrum [rubis_1.txt](Data/rubis_1.txt), normalize it to [0,1] and plot it
...
```{r, warnings=FALSE, error=TRUE}
# Load [PPC60_G_01.txt](Data/PPC60_G_01.txt) and plot it
d <- read.table("Data/rubis_1.txt", header=FALSE, col.names=c("w", "Int"))
norm01 <- function(x) {(x-min(x))/(max(x)-min(x))}
