...
Shiny applications must however be ran into Rstudio (you cannot output a standalone html document), or published onto [https://shiny.rstudio.com/](https://shiny.rstudio.com/).
...
A shiny application is a R file containing 3 elements:
...
- `sliderInput("name", "text to display", min=min_value, max=max_value, value=current_value, step=step_value)` creates a slider to input a value. This value will be retrieved by `input$name` in the `server()` function.
- `plotOutput("name")` plots the result of `output$name` defined in the `server()` function.
...
Create a new shiny app with the following, and play around with it. The input file should be the tidy <a href="Data/population.txt" download target="_blank">population.txt</a>.
...
ui <- fluidPage(
    titlePanel("City populations in France"),
    sidebarLayout(
        sidebarPanel(
        ...
        mainPanel(
            tabsetPanel(
                tabPanel("Plot", plotOutput("cityplot", height = "800px")),
...
server <- function(input, output, session) {
    ...
    output$cityplot <- renderPlot({
        ...
        ggplot(data=subset(df, city %in% input$sel_city), 
